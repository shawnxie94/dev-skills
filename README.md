...
| 子智能体编排 | `subagent-orchestration` | 需要选择子智能体角色、Runtime 适配、上下文卸载、foreground/background、steer/stop/resume 或失败恢复策略时。 | 角色契约、Runtime adapter、生命周期和上下文卸载委派包；不替代计划、实现或最终验收。 |
...
- 轻量调研：`research`（brief）→ `write-prd` 或 `write-trd`；资料多时先用 `subagent-orchestration` 委派 `researcher`。
- 正式需求分析：`research`（brief，可选）→ `subagent-orchestration`（researcher/evidence-auditor）→ `research`（deep）→ `write-prd` → `prototype-ui`（UI 假设未验证时）→ `write-trd`。
- 多模型交叉估时：`research`（deep）→ 三个或更多 Reviewer 分别运行 `delivery-estimation`（review）→ Research Lead 运行 `delivery-estimation`（synthesis）。
- 复杂需求交付：`write-prd` → `delivery-readiness`（prd_to_trd，正式交接时）→ `write-trd` → `delivery-readiness`（trd_to_plan，正式交接时）→ `execution-delivery`（先评估并行，再生成 `batch` 或 `parallel_dag` plan）→ `delivery-readiness`（plan_to_build，进入实现前）→ `subagent-orchestration`（选择角色与 Runtime adapter）→ `execution-delivery`（delegate，选择当前会话或子智能体）→ `implement-plan` → reviewer/verifier 整体验收/集成验收 → `prepare-commit` → `release-delivery`（获得对应审批后）。readiness 门是显式阶段门，非正式小改动不进 gate。
...
- Research Lead：绑定 `research`（brief）和 `delivery-estimation`（synthesis），负责研究契约、Reviewer 隔离、差异复核和最终汇总；资料或日志过大时通过 `subagent-orchestration` 卸载采集和证据核验。
