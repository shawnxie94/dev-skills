...
- Orchestration: `orchestration_mode` is `batch` or `parallel_dag`; `execution_target` is `current_session` or `subagent` once delegate routing is selected; `execution_backend` is required for a subagent and is `codex_subagent` or `zcode_mcp`. These are distinct from agent-brain's `execution_mode` task lane and the lower-level `parallel_mode` worktree taxonomy.
