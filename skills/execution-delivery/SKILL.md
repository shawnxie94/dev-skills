...
- Orchestration: `orchestration_mode` is `batch` or `parallel_dag`; `execution_target` is `current_session` or `subagent` once delegate routing is selected; `execution_backend` is required for a subagent and is `zcode_subagent`, `codex_subagent`, or `zcode_mcp`. The backend is fixed by the current harness, not free choice: a ZCode session (native `Agent` tool present) must use `zcode_subagent` and must not spawn Codex children or wrap subagents in MCP calls; `codex_subagent` requires the Codex `multi_agent_v1` tools; `zcode_mcp` applies only when a Codex session drives ZCode through its MCP bridge. These are distinct from agent-brain's `execution_mode` task lane and the lower-level `parallel_mode` worktree taxonomy.
