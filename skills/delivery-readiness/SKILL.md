...
description: Assess whether a delivery artifact is complete and safe to hand off across PRD, TRD, execution plan, implementation, verification, and release（交付就绪、阶段交接、上线前检查）. Use at formal stage handoffs or high-risk releases; not for small internal changes or as a pre-step before every stage.
---
...
| `plan_to_build` | approved canonical plan, stable plan ID, base commit, current plan SHA, selected `orchestration_mode` (`batch` or `parallel_dag`), batch sequence or DAG/dependencies as appropriate, write ownership, allowed paths, acceptance IDs, verification commands, and Task Pack linkage; when a subagent target is selected, a supported `execution_backend` |
